...
class TFDetector(object):
    ...
    def detect(self, image, mark=False):
        with self.graph.as_default():
            ...
            output_dict = self.sess.run(tensor_dict, feed_dict={
            ...
            output_dict['detection_classes'] = output_dict[
                'detection_classes'][0].astype(np.uint8)[:num]
            output_dict['detection_boxes'] = output_dict[
                'detection_boxes'][0][:num]
            output_dict['detection_scores'] = output_dict[
                'detection_scores'][0][:num]
            ...
            if mark:
                for i in range(output_dict['num_detections']):
                    ...
                    box_ymin = int(output_dict['detection_boxes'][
                                   i][0] * image_height)
                    box_xmin = int(output_dict['detection_boxes'][
                                   i][1] * image_width)
                    box_ymax = int(output_dict['detection_boxes'][
                                   i][2] * image_height)
                    box_xmax = int(output_dict['detection_boxes'][
                                   i][3] * image_width)
                    cv2.rectangle(image, (box_xmin, box_ymin),
                                  (box_xmax, box_ymax), (0, 255, 0), 3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--model_path', type=str,
                        help='path to the pd file')
    parser.add_argument('-l', '--label_path', type=str,
                        default='label_utils/mscoco_label_map.pbtxt',
                        help='label path')
    FLAGS, unparsed = parser.parse_known_args()

    category_index, image_paths = load_image_and_labels(
        FLAGS.label_path, FLAGS.image_path)
    detector = TFDetector(FLAGS.model_path, category_index)
